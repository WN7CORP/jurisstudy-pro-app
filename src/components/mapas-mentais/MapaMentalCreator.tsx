...
import { MapNode, MapaMental } from "@/types/supabase";
...
interface MapaMentalCreatorProps {
  onCreateMap?: (data: MapaMental) => void;
}
...
export const MapaMentalCreator: React.FC<MapaMentalCreatorProps> = ({ onCreateMap }) => {
  ...
  const saveMapToDatabase = async () => {
    ...
    try {
      ...
      const mapData = {
        titulo: generatedMap.titulo,
        area_direito: generatedMap.area_direito,
        estrutura: generatedMap.estrutura,
        criado_por: userId,
        criado_por_ia: true,
        publico: false
      };
  ...
  const downloadMapAsJson = () => {
    ...
    const jsonString = JSON.stringify(generatedMap.estrutura, null, 2);
            ...
            <div>
              ...
              <div className="bg-secondary/30 p-4 rounded-md text-sm overflow-auto max-h-60">
                <pre className="whitespace-pre-wrap">
                  {JSON.stringify(generatedMap.estrutura, null, 2)}
                </pre>
              </div>
            </div>
          </div>
